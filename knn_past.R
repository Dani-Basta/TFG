#' Predicts values of the time series using k-nearest neighbors algorithm. Values corresponding to instants from initial + 1 to
#' the last one are predicted. The first value predicted, which corresponds to instant initial + 1, is calculated using instants
#' from 1 to instant initial; the second value predicted, which corresponds to instant initial + 2, is predicted using instants from
#' 1 to instant initial + 1; and so on until the last value, which corresponds to instant n (length of the given time series),
