...
knn_optim_parallelf2 = function(x, k, d, v = 1, init, error_metric = "MAE", weight = "proximity", threads = 3, file, rows){
  ...
  all_predictions <- foreach(i = 1:ds, .combine = cbind) %:% foreach(j = (n - init + 1):2, .combine = cbind) %dopar% {
    ...
    num_of_file <- ((j - 1) %/% rows) + 1
    distances_matrix_size <- n - d[i] - rows * (num_of_file - 1)
    j_in_file <-((j - 1) %% rows + 1)
    initial_index <- distances_matrix_size * (j_in_file - 1) - j_in_file * (j_in_file - 1) / 2 + 1
    distances_col <- readRDS(paste0(file, d[i], "_", num_of_file))[initial_index:(initial_index + n - d[i] - j)]
