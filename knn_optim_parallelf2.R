...
knn_optim_parallelf2 = function(x, k, d, v = 1, error_metric = "MAE", weight = "proximity", threads = 3, file, rows = 1){
  ...
  raw_preds <- foreach(act_row = 2:(n - init + 1)) %:% foreach(act_d = iter(d)) %do% {
    ...
    # Read from file
    num_of_file <- ((act_row - 1) %/% rows) + 1
    i <- 1
    act_column_length <- n - act_row + ((act_row - 1) %% rows) - act_d + 1
    j <- 1
    row_in_file <- ((act_row - 1) %% rows) + 1
    while (j < row_in_file) {
    ...
    distances_element <- readRDS(paste0(paste0(file, act_d), num_of_file))[i:(i + n - act_row - 1 - act_d + 1)]
    ...
    for (k_index in 1:ks) {
      ...
      preds[k_index] <- weighted.mean(y[n - act_row + 2 - k_nn, v], weights)
    }
    
    list(d_index = d_index, instant_index = n - act_row - init + 2, preds = preds)
  }
