...
#' If a knn model is provided as the first argument, knn_forecast will be directly called. 
#' If single values are provided as k and d as no parameter search can be perfomed, knn_forecast will be called automatically
#' If no values are provided for k and/or d, values 1 to 50 will be used by default.
#' 
#' @param y A time series or a trained kNN model generated by the knn_param_search_function. In case that a model is provided
#' the knn_forecast function will be automatically called.
#' @param k Values of k's to be analyzed or chosen k for knn forecasting. Default value is 1 to 50.
#' @param d Values of d's to be analyzed or chosen d for knn forecasting. Default value is 1 to 50.
...
knn <- function(y, k = 1:50, d = 1:50, distance = "euclidean", error_measure = "MAE", weight = "proportional", v = 1, threads = 1) {
    ...
    if (length(k) == 1 && length(d) == 1) {
        warning("k and d are single integers: supposing simple prediction is required", immediate. = TRUE)
        knn_forecast(y = y, k = k, d = d, distance = distance, weight = weight, v = v, threads = threads)
    }
    else{
        warning("Beginning parameter search process, this may take a while", immediate. = TRUE)
