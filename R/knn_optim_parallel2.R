...
knn_optim_parallel2 <- function(y, k, d, v = 1, init = NULL, distance_metric = "euclidean", error_metric = "MAE", weight = "proximity", threads = NULL){
  ...
  errors_matrix <- foreach::foreach(i = 1:ds, .combine = cbind, .packages = c("forecast", "parallelDist"), .export = "knn_elements") %dopar% {
    ...
    for (k_index in 1:ks) {
      errors[k_index] <- forecast::accuracy(stats::ts(predictions[k_index, ]), real_values)[error_type]
    }
